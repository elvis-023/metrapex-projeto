...
- **Paleta**: base "papel" (fundo quase-branco com um fio de calor, não branco clínico) no light mode; grafite (nunca preto puro) para texto e superfícies elevadas (`--foreground`, `--card`, `--sidebar`) em ambos os modos — exceto o **fundo de página do dark mode** (`--background: oklch(0 0 0)`), que é preto puro por decisão explícita, não grafite. Com **uma** accent color de marca — `#009FFC` (família `oklch(_ 0.17 246)`, ajustada por modo: mais escura no light mode pra manter contraste AA com texto branco, quase pura no dark mode) — para CTAs, estados ativos, logo e pipeline. Cores semânticas separadas da accent color (verde=convertido, âmbar=expirando, vermelho=expirado) para não colidir com a marca.
- **Tipografia**: Syne (700/800) para headings e display, DM Sans (300–700) para corpo/UI (ver `app/layout.tsx`). Números (preços, totais, número do orçamento) em **IBM Plex Mono** com tabular figures, usada como voz utilitária para dado tabular.
- **Logo**: componente `<Logo size="lg|md|sm" />` (`components/shared/logo.tsx`) — "Trezo" em foreground + "fy" na accent color, Syne 800, com linha animada (`flowPulse`) embaixo do wordmark. Usado na sidebar, topo de auth/onboarding, header/footer da landing. `app/(marketing)/opengraph-image.tsx` não pode consumir o componente (Satori só aceita estilo inline) — mantém marcação própria, espelhada manualmente.
...
- **Animação**: entrada por scroll (`components/shared/animate-on-scroll.tsx`, fade+slide via Intersection Observer nativo) e hover discreto em card/botão são os padrões de movimento aplicados hoje no painel e na landing. `orbFloat`, `waveScroll`, `caseFloat` e as utilities de glassmorphism (`.glass-card`) existem em `app/globals.css` como infraestrutura do catálogo de animações do design scope, mas **não estão aplicadas a nenhuma tela** — decidir onde (ou se) usá-las é escopo de um milestone futuro, não automático por existirem. Toda animação respeita `prefers-reduced-motion`.
- **Motivo de assinatura**: o hero da landing page (`components/marketing/quote-receipt.tsx`) renderiza um orçamento real — borda perfurada de talão, numeração sequencial, linhas em mono, selo de carimbo que "prensa" uma vez ao carregar (respeita `prefers-reduced-motion`). Reservar esse tipo de gesto para um único lugar por tela — não replicar o carimbo/perfuração em todo componente. A linha animada do logo é a exceção deliberada: é o wordmark, não o carimbo, e por design aparece em todo lugar onde o logo aparece.
